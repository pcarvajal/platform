...
| `env.string()` / `.number()` / `.boolean()` / `.enum(values)` | Builder propio, sin dependencias de terceros. Cada uno devuelve un `EnvValueSchema` que soporta `.optional()` y `.default(value)` encadenados.                                                                                                                                                                          | Uso por defecto cuando el proyecto no necesita validaciones más allá de tipo/presencia/enum.                                |
| `env.object(shape)`                                           | `<S extends Shape>(shape: S) => StandardSchemaV1<Record<string, string \| undefined>, InferShape<S>>`. Devuelve un `StandardSchemaV1` plano — no un `EnvValueSchema`, así que `.optional()`/`.default()` no existen sobre el resultado de `object()` en sí (sí sobre cada campo de `shape`, si son primitivos `env.*`). | Agrupar varias vars relacionadas bajo una clave, o como base de `env.appContext` (ver abajo).                               |
