...
| `buildMessageEnvelope(overrides?)` (`@platform/testing`)          | `<TBody = unknown>(overrides?: Partial<MessageEnvelope<TBody>>) => MessageEnvelope<TBody>`. Defaults: `id` (`randomUUID()`), `source: "test-source"`, `receivedAt: new Date()`, `attributes: {}`, `body: undefined`.                                                                                                   | Test de una `MessageRoute`/consumer sin armar el `MessageEnvelope` completo a mano — contraparte de `buildHttpRequest` (ver [`testing.md`](./testing.md)) para el eje async.            |
        ...
        TransactItems: [
          ...
          ...events.map((event) => ({
            Put: {
              TableName: this.tableName,
              Item: {
                // `eventId`/`occurredOn` son opcionales en el tipo (ver tabla arriba) pero el
                // constructor de `DomainEvent` siempre los autogenera si no se pasan, así que en
                // runtime nunca son `undefined` — de ahí el `!`.
                pk: `OUTBOX#${event.eventId!}`,
