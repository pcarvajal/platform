...
contrato público cambia — antes de volver a copiar un paquete, revisa su `CHANGELOG.md` para saber
qué cambió desde tu última copia y si hay algo breaking que actualizar en tu proyecto.
...
solo en `data` sin armarlo a mano) **más `cause`, si se pasó uno** (serializado como
...
`private readonly cardToken: string` en el constructor de un error propio **sí** termina en
`data` igual que una `public`. No pases secretos/tokens/PII como propiedad de un error; si
necesitás preservarlos para debugging interno, filtralos antes de loguear `toScalars()`.
...
| `DomainError`                                        | `abstract class DomainError extends ExtensibleError` (ver [Jerarquía de errores](#jerarquía-de-errores)) con `abstract readonly type: string`; constructor `(message, { details?, cause? })`; hereda `toScalars()` de `StructuredError`, que serializa `{ type, origin?, description, data }` **reflejando automáticamente** las propiedades propias del error (excluye `type`, `origin` y `message`) más `cause`/`details` si se pasaron.                                                                     | Por eso el patrón correcto es declarar las propiedades adicionales del error como `readonly` en el constructor (`constructor(readonly orderId: string) { super(...) }`) — `toScalars()` las recoge solo sin necesidad de armar `data` a mano.                                                                                                                                                           |
...
Interfaz (`packages/infrastructure/src/ports/EventBus.ts`): `publish(events: DomainEvent[]): Promise<void>`
...
Interfaz genérica (`packages/infrastructure/src/ports/Cache.ts`): `get(key): Promise<T | undefined>` /
...
`Cache<T>`, `RestClient` y `EventBus` son interfaces "puerto" (contrato tecnología-agnóstico
implementado por un adapter concreto) y viven en `infrastructure` — es la regla por defecto. `Logger`
(`packages/core/src/application/Logger.ts`) es la única excepción, y vive en `core/application` en
vez de `infrastructure`, por una razón puntual: un `UseCase` necesita loguear **directo**, dentro de
`execute()`, y `core/application` no puede importar de `infrastructure` (`boundaries/element-types`
en `@platform/eslint-config` solo permite `application → domain`). Si `Logger` viviera en
`infrastructure`, ningún caso de uso podría inyectarlo sin romper la dirección de dependencias.
...
igual que `Logger`. Ante la duda, preguntate: "¿un `UseCase` llama a este puerto dentro de su propio
`execute()`, o alguien se lo pasa ya resuelto desde afuera?". Lo primero → `core`; lo segundo →
`infrastructure`.
