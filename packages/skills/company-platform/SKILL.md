...
| `RedisHttpResponseCache` (adapter-redis)                                                                    | `implements HttpResponseCache`; constructor `(client: Redis, { keyPrefix?, logger? })` — recibe un cliente `ioredis` ya conectado, no lo crea. Es un cache, no una fuente de verdad: si Redis falla o una entrada está corrupta, degrada a cache miss (logueado vía `logger`, si se pasa) en vez de propagar el error a la request.                                                                                                                                                                                                                                                                                                       | `new RedisHttpResponseCache(new Redis(config.REDIS_URL), { logger })` en `infrastructure/deployment/*`, pasado a `withHttpCache` al armar las `routes`.                                                                                                                                  |
